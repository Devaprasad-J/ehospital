...
class MedicineBill(models.Model):
    prescription = models.ForeignKey(EPrescription, on_delete=models.CASCADE, related_name='medicine_bills')
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"Bill for {self.patient.user.username} - {self.payment_status}"
